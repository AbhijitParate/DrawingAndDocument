...
var CanvasButton = fabric.util.createClass(fabric.Object, fabric.Observable, {
    type:'button',
    H_PADDING: 20,
    V_PADDING: 50,
    originX: 'center',
...
$(document).ready(function() {
    ...
    function addButton(point) {
        var button = new CanvasButton('images/icons/bullhorn.png', {
            top: point.y,
            left: point.x,
            // lockMovementX: true,
            // lockMovementY: true,
        ...
        button.on('image:loaded', canvas.renderAll.bind(canvas));
        canvas.add(button);
        // let activeObject = canvas.getActiveObject();
        // let group = new fabric.Group([activeObject, button]);
        // canvas.add(group);
        // canvas.renderAll();
    ...
    canvas.on('mouse:down', function (e) {
        if(e.target){
            switch (e.target.get('type')){
                case 'button':
                    console.info("Audio Button clicked");
                    break;
            }
        }
    });
    ...
    $(function() {
        ...
        $(document).contextmenu({
            ...
            beforeOpen: function (event, ui) {
                ...
                $(document).contextmenu("setEntry", "link", "Link external file")
                    .contextmenu("enableEntry", "paste","Paste");
            ...
            select: function (event, ui) {
                ...
                switch (ui.cmd) {
                    case "link":
                        ...
                        console.info("Link clicked at x->" + clickPoint.x + " y->" + clickPoint.y);
                        // let point
                        //     = new fabric.Point(
                        //         canvas.getActiveObject().left + clickPoint.x,
                        //         canvas.getActiveObject().top + clickPoint.y );
                        var pointer = canvas.getPointer(event.originalEvent);
                        addButton(pointer);
                        break;
                    case "paste":
                        // Code to paste file
                        console.info("Paste clicked at x->" + clickPoint.x + " y->" + clickPoint.y);
                        break;
                }
...
// Creates dialog to attach notes
    ...
    // Creates dialog to attach notes
    $("#dialog-upload-progress").dialog({
        resizable: false,
        height: "auto",
        width: "auto",
        position: {my: 'top', at: 'top+200'},
        open: function () {
            console.info("Files upload dialog opened");
        },
        close: function () {
            console.info("Files upload dialog closed");
        },
        modal: true,
        autoOpen: false,
        buttons: {
    ...
    $("#status-save").click(function () {
        $("#dialog-upload-progress").dialog("open");
        for (let i = 0; i < attachments.length; i++) {
